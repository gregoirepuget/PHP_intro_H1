<?php
/*
  header part. Use for whole site.
*/
?>
  <header id="header">
   <ul class="menu">
     <li><a href="#" title="Item 1">Item 1</a></li>
     <li><a href="#" title="Item 2">Item 2</a></li>
     <li><a href="#" title="Item 3">Item 3</a></li>
     <li><a href="#" title="Item 4">Item 4</a></li>
     <li><a href="#" title="Item 5">Item 5</a></li>
     <li><a href="#" title="Item 6">Item 6</a></li>
     <li><a href="#" title="Item 7">Item 7</a></li>
     <li><a href="#" title="Item 8">Item 8</a></li>
     <li><a href="#" title="Item 9">Item 9</a></li>
     <li><a href="#" title="Item 10">Item 10</a></li>
   </ul>
 </header>